...
RSpec.describe Pina::UploadedDocumentPairing do
  describe 'find' do
    it 'finds pairing by id' do
      VCR.use_cassette 'uploaded_document_pairing_find', match_requests_on: [:method, :path] do
  ...
  describe 'create' do
    it 'creates new uploaded document pairing' do
      VCR.use_cassette 'uploaded_document_pairing_create', match_requests_on: [:method, :path] do
